...
#    adj_neightborhood.setdiag(4,0)
    ...
    if sentence_length_underlying < 40:
...
#                      "train_acc=", "{:.5f}".format(outs[2]), "val_loss=", "{:.5f}".format(cost_val_loss[-1]), "val_F1=", "{:.5f}".format(cost_val_f1[-1]),"time=", "{:.5f}".format(time.time() - t))
        ...
        if epoch_h > FLAGS.early_stopping and cost_val_loss[-1] > np.mean(cost_val_loss[-(FLAGS.early_stopping+1):-1]):
            ...
            test_f1 = cmPRF(cm,ncstart=1)[5]
            test_loss = outs_test_out[0]   
            print("Early stopping...","test_loss=", "{:.5f}".format(test_loss),"test_f1=", "{:.5f}".format(test_f1))
            break
...
np.save("/home/yld8809/cm_tp_gcn_nei_50", cm)
np.save("/home/yld8809/cost_val_f1_tp_gcn_50",cost_val_f1)
np.save("/home/yld8809/cost_val_loss_tp_gcn_50",cost_val_loss)