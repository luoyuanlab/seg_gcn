...
ind_dict1 = np.float32(features1[:,1:7])
idx1 = unique_ind(ind_dict1)


features2 = np.load('/home/yld8809/word_embedding_tp_all_test.npy')
ind_dict2 = np.float32(features2[:,1:7])
idx2 = unique_ind(ind_dict2)

unique_word_index = np.vstack((ind_dict1[idx1,:],ind_dict2[idx2,:]))
...
all_info = np.vstack((label1_info,label2_info))
idx_all = unique_ind(all_info)
features = np.load('/home/yld8809/tp_features_padded.npy')
features = features[idx_all]

adj = np.load('/home/yld8809/tp_adj_padded.npy')
adj = adj[idx_all]
...
for epoch in range(0,current_ind_list.shape[0]):
    ...
    if word_ind_sorted[0] == int(temp_embedding_ind[2]):
        ...
        if word_ind_sorted[0] != 0:
            all_phrase_val[epoch, 0:word_ind_sorted[0],0] = 3
        if word_ind_sorted[1]+1 != word_ind_sorted[2]:
            all_phrase_val[epoch, int(word_ind_sorted[1]+1):int(word_ind_sorted[2]),0] = 1       
        ...
        for epoch_m in shuffle_ind_per_epoch:
            ...
            for epoch in range(0,current_ind_list.shape[0]):
                ...
                if word_ind_sorted[0] == int(temp_embedding_ind[2]):
                    ...
                    if word_ind_sorted[0] != 0:
                        all_phrase_par[epoch, 0:word_ind_sorted[0],0] = 3
                    if word_ind_sorted[1]+1 != word_ind_sorted[2]:
                        all_phrase_par[epoch, int(word_ind_sorted[1]+1):int(word_ind_sorted[2]),0] = 1   
...
for epoch in range(0,current_ind_list.shape[0]):
    ...
    if word_ind_sorted[0] == int(temp_embedding_ind[2]):
        ...
        if word_ind_sorted[0] != 0:
            all_phrase_test[epoch, 0:word_ind_sorted[0],0] = 3
        if word_ind_sorted[1]+1 != word_ind_sorted[2]:
            all_phrase_test[epoch, int(word_ind_sorted[1]+1):int(word_ind_sorted[2]),0] = 1     
...
np.save("/home/yld8809/cm_tp_sgc", cm)
np.save("/home/yld8809/cost_val_f1_tp_sgc",cost_val_f1)
np.save("/home/yld8809/cost_val_loss_tp_sgc",cost_val_loss)