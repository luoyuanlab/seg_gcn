...
rel_summary_all_doc = np.load("/home/yld8809/all_rel/pp_all_train.npy")
raw_doc_folder = "/home/yld8809/all_rel/txt_all_train/"

rel_summary_all_doc_test = np.load("/home/yld8809/all_rel/pp_all_test.npy")
raw_doc_folder_test = "/home/yld8809/all_rel/txt_all_test/"

model_size = 300
model = gensim.models.KeyedVectors.load_word2vec_format("/home/yld8809/semrel/mimic3_pp"+str(model_size)+".txt", binary=False)  
...
for i in range(0,num_doc):
    ...
    for j in index:
        ...
        max_size = max(max_size,np.asarray(sentence_word_m).shape[0])

count = -1
for i in range(349,349+num_doc_test):
    count = count + 1
    raw_doc_dir = raw_ind_test[count] # raw doc 
    
    text_curr = open(raw_doc_dir,encoding='utf-8').read()
    text_curr = text_curr.split('\n')
    index = np.asarray(np.where([int(rel_summary_all_doc_test[k,2]) == i for k in range(0,len(rel_summary_all_doc_test[:,2]))])).reshape(-1,1)
    
    for j in index:
        # word embedding
        sentence = text_curr[int(np.asscalar(rel_summary_all_doc_test[j,3]))-1]
        sentence_word_m = sentence.split()
        max_size = max(max_size,np.asarray(sentence_word_m).shape[0])
...
for i in range(0,num_doc):
    ...
    text_curr = open(raw_doc_dir).read()
    text_curr = text_curr.split('\n')
    index = np.asarray(np.where([int(rel_summary_all_doc[k,2]) == i for k in range(0,len(rel_summary_all_doc[:,2]))])).reshape(-1,1)
...
np.save("/home/yld8809/pp_features_padded_train" + str(model_size), features_tuple)
np.save("/home/yld8809/pp_adj_padded_train" + str(model_size), adj_tuple)
np.save("/home/yld8809/sentence_length_tuple_train_pp" + str(model_size), sentence_length_tuple)
np.save("/home/yld8809/max_size_pp" + str(model_size), max_size)